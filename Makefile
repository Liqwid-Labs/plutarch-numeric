...
usage:
	...
	@echo "  watch [COMMAND]     -- Track files: plutarch-numeric.cabal, src/* test/* testlib/* and run 'make [COMMAND]' on change"
	@echo "  test                -- Run cabal v2-test"
	@echo "  ghci                -- Run cabal v2-repl plutarch-numeric"
...
hoogle: requires_nix_shell haddock
	-killall -q hoogle
	hoogle generate --local=haddock --database=plutarch-numeric.hoo
	hoogle server --local --database=plutarch-numeric.hoo -p 8070 >> /dev/null && echo $! > ./.hoogle.pid &
...
watch: requires_nix_shell
	while sleep 1;																				\
	do	                                                  \
	  find src testlib test plutarch-numeric.cabal |   \
...
ghci: requires_nix_shell
	cabal v2-repl $(GHC_FLAGS) plutarch-numeric
